...
if (saveFeedbackAndDataBtn) {
    saveFeedbackAndDataBtn.addEventListener('click', () => {
        ...
        for (const cluster of experimentData.clusters) {
            if (!cluster.feedback || !cluster.feedback.reasonCreated?.trim() || !cluster.feedback.meaning?.trim() || !cluster.feedback.reasonName?.trim()) {
                allProvided = false;
                break;
            }
        }
        ...
        submitAndFinishBtn.addEventListener('click', async (e) => {
            ...
            try {
                const gasWebAppUrl = 'https://script.google.com/macros/s/AKfycbzB474v5g3S9_tR6o_X6jiFdxS64keEFhV97AA7Jw5K5wOqF9eD1IDsQd-J8Sseni6syg/exec'; // ★★★要書き換え★★★
