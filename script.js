...
let foodList = [
    ...
    { name: "empanada", label: "エンパナーダ", imgSrc: "51HpEz7bykL.jpg_BO30,255,255,255_UF900,850_SR1910,1000,0,C_QL100_.png", info: "アルゼンチン伝統料理：具材をパイ生地で包んで焼いた料理。具材をパイ生地で包んで焼いた料理。本場の味わいを楽しめるエンパナーダの6個セット。南米の伝統的な味を日本で堪能できます。\n食べ比べセット：シェフが厳選した６種類のエンパナーダを一度に楽しめる贅沢な詰め合わせパック" },
...
function showInstructionScreen(title, bodyHtml, buttonText, nextAction) {
    ...
    let instructionContinueBtn = document.getElementById('instruction-continue-btn');
    ...
    // 古いイベントリスナーを削除するためボタンを複製して置き換える
    const newBtn = instructionContinueBtn.cloneNode(true);
    if (instructionContinueBtn.parentNode) {
        instructionContinueBtn.parentNode.replaceChild(newBtn, instructionContinueBtn);
    }
    instructionContinueBtn = newBtn;
    
    // 新しいボタンに次のアクションを設定
    instructionContinueBtn.addEventListener('click', nextAction);
    
    showScreen(document.getElementById('screen2'));
}
